...
final class TopBarNode: SKNode {
    ...
    private let innerBandNode = SKSpriteNode(color: SKColor(white: 1.0, alpha: 0.05), size: .zero)
    ...
    func layout(width: CGFloat, height: CGFloat) {
        ...
        backgroundNode.texture = TextureFactory.shared.cardTexture(size: backgroundSize, style: .overlay)
        backgroundNode.size = backgroundSize
        backgroundNode.position = .zero
        innerBandNode.size = CGSize(width: width * 0.9, height: max(8, clampedHeight * 0.18))
        innerBandNode.position = CGPoint(x: 0, y: clampedHeight * 0.16)
        ...
        let titleX = -width / 2 + LevelSelectStyle.Metric.cardInnerPadding + backButton.size.width + 14
        let maxTitleWidth = width - (LevelSelectStyle.Metric.backButtonWidth + LevelSelectStyle.Metric.cardInnerPadding + 24) - 18
        ...
        titleLabel.position = snap(CGPoint(x: titleX, y: 1))
    }
